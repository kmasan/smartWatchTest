...
class WatchGyroscope {
    private var queue: LinkedList<GyroscopeData> = LinkedList() //キュー
    ...
    fun queueAdd(time: Long,x: Float,y: Float,z: Float){
        //キューへのデータ追加
        queue.add(GyroscopeData(time, x, y, z))
    }
    ...
    fun csvWriter(fileName: String, path: String): Boolean {
        ...
        try{
            //書込み先指定
            val writer = FileWriter(path.plus("/").plus(fileName).plus("_watchGyro.csv"))
